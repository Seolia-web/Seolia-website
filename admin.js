...
function openQuoteModal() {
  ...
  document.getElementById('quote-setup').value = contact?.prix_setup || '';
  document.getElementById('quote-monthly').value = contact?.prix_mensuel || '';
  document.getElementById('quote-formule').value = contact?.formule || '';
...
function generateQuotePDF() {
  ...
  const setupTVAC = parseFloat(document.getElementById('quote-setup').value) || 0;
  const monthlyTVAC = parseFloat(document.getElementById('quote-monthly').value) || 0;
  const formule = document.getElementById('quote-formule').value || '';
  const setupHTVA = setupTVAC / TVA_RATE;
  const setupTVA = setupTVAC - setupHTVA;
  const monthlyHTVA = monthlyTVAC / TVA_RATE;
  const monthlyTVA = monthlyTVAC - monthlyHTVA;
  const totalPremier = setupTVAC + monthlyTVAC;
  const totalHTVA = setupHTVA + monthlyHTVA;
  const totalTVA = setupTVA + monthlyTVA;

  const printZone = document.getElementById('print-zone');
  printZone.style.display = 'block';
  printZone.innerHTML = `
    <div style="max-width:700px;margin:0 auto;font-family:Inter,sans-serif;color:#1e293b;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:40px;border-bottom:3px solid #00d68f;padding-bottom:20px;">
        <div>
          <div style="font-size:32px;font-weight:900;color:#0a1628;letter-spacing:-1px;">Seo<span style="color:#00d68f">lia</span></div>
          <div style="color:#64748b;font-size:13px;margin-top:4px;">Agence Web — contact@seolia.be</div>
        </div>
        <div style="text-align:right;">
          <div style="font-size:22px;font-weight:800;color:#0a1628;">DEVIS</div>
          <div style="color:#64748b;font-size:13px;">Date : ${formatDate(dateStr)}</div>
        </div>
      </div>
      <div style="margin-bottom:30px;">
        <div style="font-size:13px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;">Client</div>
        <div style="font-size:16px;font-weight:700;">${esc(contact?.nom||'-')}</div>
        <div style="color:#64748b;">${esc(contact?.entreprise||'')}</div>
        <div style="color:#64748b;">${esc(contact?.adresse||'')}${contact?.code_postal?' — '+contact.code_postal:''}</div>
        <div style="color:#64748b;">${esc(contact?.email||'')}</div>
      </div>
      <div style="margin-bottom:30px;">
        <div style="font-size:13px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;">Prestation</div>
        <div style="font-size:15px;font-weight:600;">${esc(formule)}</div>
      </div>
      <table style="width:100%;border-collapse:collapse;margin-bottom:30px;">
        <thead>
          <tr style="background:#f5f7fa;">
            <th style="padding:10px 14px;text-align:left;font-size:12px;font-weight:700;color:#64748b;text-transform:uppercase;border-bottom:1px solid #e8edf3;">Description</th>
            <th style="padding:10px 14px;text-align:right;font-size:12px;font-weight:700;color:#64748b;text-transform:uppercase;border-bottom:1px solid #e8edf3;">HTVA</th>
            <th style="padding:10px 14px;text-align:right;font-size:12px;font-weight:700;color:#64748b;text-transform:uppercase;border-bottom:1px solid #e8edf3;">TVA 21%</th>
            <th style="padding:10px 14px;text-align:right;font-size:12px;font-weight:700;color:#64748b;text-transform:uppercase;border-bottom:1px solid #e8edf3;">TVAC</th>
          </tr>
        </thead>
        <tbody>
          ${setupTVAC > 0 ? `<tr><td style="padding:12px 14px;border-bottom:1px solid #f1f5f9;">Frais de setup (une fois)</td><td style="padding:12px 14px;text-align:right;border-bottom:1px solid #f1f5f9;">${fmtEur(setupHTVA)}</td><td style="padding:12px 14px;text-align:right;border-bottom:1px solid #f1f5f9;">${fmtEur(setupTVA)}</td><td style="padding:12px 14px;text-align:right;border-bottom:1px solid #f1f5f9;font-weight:700;">${fmtEur(setupTVAC)}</td></tr>` : ''}
          ${monthlyTVAC > 0 ? `<tr><td style="padding:12px 14px;border-bottom:1px solid #f1f5f9;">Abonnement mensuel</td><td style="padding:12px 14px;text-align:right;border-bottom:1px solid #f1f5f9;">${fmtEur(monthlyHTVA)}</td><td style="padding:12px 14px;text-align:right;border-bottom:1px solid #f1f5f9;">${fmtEur(monthlyTVA)}</td><td style="padding:12px 14px;text-align:right;border-bottom:1px solid #f1f5f9;font-weight:700;">${fmtEur(monthlyTVAC)}</td></tr>` : ''}
        </tbody>
        <tfoot>
          <tr style="background:#f0fdf4;">
            <td colspan="3" style="padding:14px;font-weight:800;font-size:15px;color:#0a1628;">Total 1er mois (setup + abonnement)</td>
            <td style="padding:14px;text-align:right;font-weight:900;font-size:18px;color:#00d68f;">${fmtEur(totalPremier)}</td>
          </tr>
          <tr>
            <td colspan="3" style="padding:8px 14px;font-size:13px;color:#64748b;">Puis mensuel récurrent</td>
            <td style="padding:8px 14px;text-align:right;font-size:13px;color:#64748b;">${fmtEur(monthlyTVAC)}/mois</td>
          </tr>
        </tfoot>
      </table>
      <div style="background:#f5f7fa;border-radius:12px;padding:16px;font-size:12px;color:#64748b;margin-bottom:30px;">
        <strong style="color:#1e293b;">Conditions :</strong> Ce devis est valable 30 jours. Prix TVAC (TVA 21% incluse). Paiement à 30 jours après facturation.
      </div>
      <div style="display:flex;justify-content:space-between;margin-top:40px;padding-top:20px;border-top:1px solid #e8edf3;">
        <div><div style="font-size:12px;color:#64748b;margin-bottom:30px;">Signature client</div><div style="border-bottom:1px solid #0a1628;width:200px;"></div></div>
        <div style="text-align:right;font-size:12px;color:#64748b;">
          <div style="font-weight:700;color:#0a1628;">Seolia</div>
          <div>Belgique</div>
          <div>contact@seolia.be</div>
        </div>
      </div>
      <div class="no-print" style="margin-top:24px;text-align:center;">
        <button onclick="window.print()" style="padding:12px 24px;background:#0a1628;color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;font-family:Inter,sans-serif;">🖨️ Imprimer / Sauvegarder PDF</button>
        <button onclick="document.getElementById('print-zone').style.display='none'" style="padding:12px 24px;background:#f1f5f9;color:#1e293b;border:none;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;font-family:Inter,sans-serif;margin-left:10px;">Fermer</button>
      </div>
    </div>
  `;
